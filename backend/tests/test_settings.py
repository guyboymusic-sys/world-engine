...
def test_celery_app_falls_back_to_redis_url(monkeypatch):
    ...
    with monkeypatch.context() as m:
        m.setenv("REDIS_URL", "redis://localhost:6379/9")
        m.delenv("CELERY_BROKER_URL", raising=False)
        m.delenv("CELERY_RESULT_BACKEND", raising=False)
        get_settings.cache_clear()
...
def test_celery_app_uses_mixed_override_and_fallback(monkeypatch):
    ...
    with monkeypatch.context() as m:
        m.setenv("REDIS_URL", "redis://localhost:6379/9")
        m.setenv("CELERY_BROKER_URL", "redis://localhost:6379/7")
        m.delenv("CELERY_RESULT_BACKEND", raising=False)
        get_settings.cache_clear()
        importlib.reload(celery_module)
...
def test_celery_app_blank_overrides_fall_back_to_redis(monkeypatch):
    from backend.core import celery_app as celery_module
    with monkeypatch.context() as m:
        m.setenv("REDIS_URL", "redis://localhost:6379/9")
        m.setenv("CELERY_BROKER_URL", "")
        m.setenv("CELERY_RESULT_BACKEND", "")
        get_settings.cache_clear()
        importlib.reload(celery_module)
